# Trace design and disagreement resolution
...
The obvious rules (“if both models agree, accept it”) are only the beginning. The real power comes from the non obvious rules — the ones that catch subtle bias, hallucination, drift, and semantic inconsistency.

The disagreement rules don’t just resolve differences — they shape the behaviour of the entire system.

They determine:
- how conservative the system is
- how much it trusts models
- how much it trusts the ontology
- how much it trusts itself
- how it handles uncertainty
- how it handles bias
- how it handles drift
- how it handles edge cases
- how it handles non traditional candidates

This is why the rules matter so much.

For example:
- If we always took the intersection of model outputs, the system would be extremely safe but too conservative — it would under infer skills for candidates with unusual backgrounds.
- If we always took the union, the system would hallucinate skills and over inflate candidates.
- If we always trusted the ontology, the system would be rigid and slow to adapt to new technologies.
- If we always trusted the models, the system would drift and become biased.

So the disagreement rules are not just “logic” — they are the behavioural DNA of the protocol.

## 1. Trace Format — The Backbone of Explainability — what we log, how we log it, and why
...
## 2. Disagreement Resolution Rules — The Real Magic - Disagreement resolution rules — the deep logic that makes multi model validation meaningful
...
Example: Model A: “React”, “Frontend Frameworks” Model B: “Frontend Frameworks” → Accept only “Frontend Frameworks”.
...
Example: Model A weight: 0.6 Model B weight: 0.4 Skill inferred by both → 1.0 Skill inferred by A only → 0.6 (below threshold → discard)
...
Example: Model A: “PostgreSQL → SQL” Model B: “PostgreSQL → Relational DB” Ontology: “PostgreSQL → Relational DB” → Accept ontology mapping.
...
Example: Model A: “collaboration” (0.7 confidence) Model B: “mentoring” (0.6 confidence) → Accept both with weighted scores.
